...
static TEE_Result check_masterkey(uint32_t param_types, TEE_Param params[4]){
	...
	DMSG("Try to execute: TEE_OpenPersistentObject"); //joao
	TEE_OpenPersistentObject(TEE_STORAGE_PRIVATE, &masterkey_ext_id, sizeof(masterkey_ext_id), flags_read, &obj);
...
static TEE_Result erase_masterkey(uint32_t param_types, TEE_Param params[4]){
	...
	if(res == TEE_SUCCESS && obj!=TEE_HANDLE_NULL){
		params[1].value.a = 1;
		DMSG("Master Key exists and will be removed!");
...
static TEE_Result get_bitcoin_address(uint32_t param_types, TEE_Param params[4]){
	...
	printf("\n Params value a: %d\n", params[1].value.a);
	printf("\n Params value b: %d\n", params[1].value.b);
	printf("\nGet_bitcoin_address of account_id: %d\n", account_id);

	printf("%d\n",account_id);
	res = get_account_address(address, account_id);

	
	printf("Bitcoin Addess is: ");
...
static TEE_Result sign_transaction(uint32_t param_types, TEE_Param params[4]){
	...
	//printf("Param 1 memref buffer: %d\nParam 1 memref size:%d\nParam 2 memref buffer:%d\naccount ID: %d\n", (uint8_t *)params[1].memref.buffer, params[1].memref.size,(uint8_t *)params[2].memref.buffer, account_id);
...
static void get_child_privatekey(uint32_t i, uint8_t* child_sk, uint8_t* child_chaincode){
	...
	// Constant used to derive the first child key level (m/0')
	...
	//printf("Extended Masterkey (BASE16) in get_child_privatekey: ");
	//print_uint8(masterkey_ext, 64);

	TEE_MemMove(masterkey, masterkey_ext, 32);
	TEE_MemMove(masterchain, masterkey_ext+32, 32);
	//printf("parent_masterkey in get_child_privatekey: ");
	//print_uint8(masterkey, 32);
	//printf("parent_masterchain in get_child_privatekey: ");
	//print_uint8(masterchain, 32);

	res = hdnode_private_ckd(masterkey, masterchain, i, child_sk, child_chaincode);

	//printf("child_sk in get_child_privatekey: ");
	//print_uint8(child_sk, 32);
	//printf("child_chaincode in get_child_privatekey: ");
...
//read masterkey and masterchain from i
static void get_child_publickey(uint32_t i, uint8_t* child_pk_x, uint8_t* child_pk_y){
	TEE_Result res;
	TEE_ObjectHandle obj = TEE_HANDLE_NULL;
...
static void from_mnemonic_to_masterkey (char* mnemonic){
	...
	print_uint8(masterchain, 32);
	printf("Extended Masterkey (BASE16): ");
	print_uint8(masterkey_ext, 64);
	printf("##########################\n\n");
}
...
static uint32_t sign_raw_tx(uint8_t *rawtx, size_t bytes, uint8_t *sig, uint32_t account_id){
	...
	get_child_privatekey(account_id, child_sk, child_cc); //generate private key
	get_child_publickey(account_id, child_pk_x, child_pk_y); //generate public key
	...
	res = TEE_DigestDoFinal(op, rawtx, bytes, hash, &hlen); //SHA256 HASH over the input value (buf in client application) //in rust they only make once
	res = TEE_DigestDoFinal(op, hash, hlen, hash, &hlen);	//SHA256 HASH over result of last HASH
	...
	res = TEE_SetOperationKey(op2, key); //utiliza tanto a secret key como a private key para assinar 
	if(res != TEE_SUCCESS){
	...
	res = TEE_AsymmetricSignDigest(op2, NULL, 0, hash, hlen, sig, &siglen);// hash = hash(hash(msg)) + signature
	if(res != TEE_SUCCESS){
...
static TEE_Result get_account_address(uint8_t *addr, uint32_t account_id){
	...
	TEE_AllocateTransientObject(TEE_TYPE_ECDSA_KEYPAIR, 256, &key);
	TEE_InitRefAttribute(attr, TEE_ATTR_ECC_PRIVATE_VALUE, child_sk, 32); //child_sk está vazio e na função de de issue a transation tem algo
	TEE_InitRefAttribute(attr+1, TEE_ATTR_ECC_PUBLIC_VALUE_X, child_pk_x, 32);
	TEE_InitRefAttribute(attr+2, TEE_ATTR_ECC_PUBLIC_VALUE_Y, child_pk_y, 32);
	TEE_InitValueAttribute(attr+3, TEE_ATTR_ECC_CURVE, TEE_ECC_CURVE_NIST_P256, 4);
	TEE_PopulateTransientObject(key, attr, 4);
...
static TEE_Result ecdsa_to_bitaddr(TEE_ObjectHandle obj, uint8_t *bcadd, uint8_t network){
	...
	TEE_MemMove(pubkey + 8, pubY, 32); //joao +8?????
	TEE_Free(pubX);
	TEE_Free(pubY);
	pubX = NULL;
	pubY = NULL;
