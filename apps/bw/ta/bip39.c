...
TEE_Result get_random_mnemonic(uint32_t strength, char* out){
	...
	printf("TEE_GenerateRandom executing ....");
	TEE_GenerateRandom(entropy, strength/*/8*/);

	res = TEE_AllocateOperation(&op, TEE_ALG_SHA256, TEE_MODE_DIGEST, 0); //esta função esta implementada no entanto é necessário dar o check
	if(res != TEE_SUCCESS) EMSG("Failed to allocate operation: 0x%x", res);

	res = TEE_DigestDoFinal(op, entropy, 32/*strength*//*/8*/, entropy_hash, &entropy_hash_len);
	if(res != TEE_SUCCESS) EMSG("Failed to DigestDoFinal: 0x%x", res);

	entropy[32] = entropy_hash[0];
...
TEE_Result from_mnemonic_to_seed(const char *mnemonic, const char *passphrase, uint8_t seed[512 / 8], void (*progress_callback)(uint32_t current, uint32_t total)){
	...
	pbkdf2_hmac_sha512_Init(&pctx, (const uint8_t *)mnemonic, strlen(mnemonic), (const uint8_t *)salt, passphraselen, 0);

	pbkdf2_hmac_sha512_Update(&pctx, BIP39_PBKDF2_ROUNDS);
